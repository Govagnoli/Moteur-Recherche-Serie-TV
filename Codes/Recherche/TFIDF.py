...
def listFicMotsVOVF():
    tous_fichiers_txt =[]
    for cheminfichier in glob(join(CHEMIN_MOTS_VO_VF, "*.txt")) : #Path(CHEMIN_MOTS_VO_VF).rglob("*.txt")
        tous_fichiers_txt.append(basename(cheminfichier))
...
def listTxtTousFic():
    ...
    for fichier_txt in tous_fichiers_txt:
        with open(join(CHEMIN_MOTS_VO_VF, fichier_txt), encoding='utf-8') as f:
            texteFichier = f.read()
...
def genererCSV_TFIDF(rep_sortie = CHEMIN_Stockage_CSV_TFIDF) :
    ...
    for index, motsSeries in enumerate(modelEtMatrice["matrice"]):
        ...
        df_mots_poids = DataFrame.from_records(tuple_mot_poids, columns=['mots', 'poids']).sort_values(by='poids', ascending=False).reset_index(drop=True)
        #supprime tout les mots ayant un poids = 0
        df_mots_poids = df_mots_poids.loc[df_mots_poids['poids'] > 0]
        nomFichier = listeFic[index]
        titreSerie  = match(r"^(.*?)_(.*?)$", nomFichier).group(1)
        df_mots_poids['serie'] = titreSerie
        # enregistrer les résultats dans un document CSV, en utilisant
        df_mots_poids.to_csv(join(CHEMIN_Stockage_CSV_TFIDF, nomFichier.replace(".txt", ".csv")))